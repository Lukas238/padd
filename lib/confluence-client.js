...
export class ConfluenceClient {
   ...
   */
  async updatePage({ pageId, title, body, version, parentId = null }) {
    const payload = {
      type: "page",
      title,
   ...
   * Get page by ID
   * @param {string} pageId
   * @param {string} expand - comma-separated expand fields
   */
  async getPage(pageId, expand = 'body.storage,version,space,ancestors') {
    return this.request("GET", `${this.apiPrefix}/content/${pageId}?expand=${expand}`);
  }

  /**
   * Get direct child pages of a page
   * @param {string} pageId
   * @returns {Promise<Array>} Array of child page stubs {id, title}
   */
  async getChildPages(pageId) {
    ...
    while (true) {
      const data = await this.request(
        'GET',
        `${this.apiPrefix}/content/${pageId}/child/page?limit=${limit}&start=${start}&expand=title`
      );
  ...
  /**
   * Get all descendants of a page up to a given depth.
   * @param {string} pageId
   * @param {number} depth - max depth (Infinity = unlimited)
   * @param {number} _currentDepth - internal, do not pass
   * @returns {Promise<Array>} Flat array of page stubs {id, title}
   */
  async getDescendants(pageId, depth = Infinity, _currentDepth = 1) {
    if (_currentDepth > depth) return [];
    const children = await this.getChildPages(pageId);
    const all = [...children];
    for (const child of children) {
      const grandchildren = await this.getDescendants(child.id, depth, _currentDepth + 1);
  ...
  /**
   * Get a lightweight list of all pages in a space (or subtree) using CQL.
   * Returns [{id, title, version: {number}, ancestors: [{id, title}]}]
   * No page body is fetched — this is a fast metadata-only call.
   *
   * @param {string} spaceKey
   * @param {string} [rootPageId] - if set, only pages under this ancestor
   */
  async getSpacePageList(spaceKey, rootPageId = null) {
    // Use ancestor CQL when rootPageId is known (more reliable on Server/DC)
    // Fall back to space CQL otherwise
    const cql = rootPageId
      ? `ancestor = ${rootPageId} OR id = ${rootPageId}`
    ...
    while (true) {
      const data = await this.request(
        'GET',
        `${this.apiPrefix}/content/search?cql=${encodeURIComponent(cql)}&expand=version,ancestors&limit=${limit}&start=${start}`
      );
      results.push(...(data.results || []));
      if (!data.results || data.results.length < limit || results.length >= (data.totalSize || 0)) break;
      start += limit;
    }
    return results;
  }
