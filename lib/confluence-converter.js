...
// Maps Confluence panel macro names → GitHub Alert types (for pull)
const PANEL_TO_ALERT = { info: 'NOTE', note: 'IMPORTANT', tip: 'TIP', warning: 'WARNING' };
...
function convertMacro(name, body) {
  switch (name) {
    ...
    case 'tip':
    case 'warning': {
      const alertType = PANEL_TO_ALERT[name];
    ...
    case 'children': {
      const depth = extractMacroParam(body, 'depth');
      const allPages = extractMacroParam(body, 'all');
      const parts = ['children'];
      if (depth) parts.push(`depth="${depth}"`);
      if (allPages) parts.push(`all="${allPages}"`);
      return `\n{{${parts.join(' ')}}}\n`;
    }

    case 'status': {
      const label = extractMacroParam(body, 'title') || '';
      const colour = extractMacroParam(body, 'colour') || extractMacroParam(body, 'color') || '';
      const parts = ['status'];
    ...
    default:
      return null; // Unknown macro → verbatim wrap in convertMacros
  }
}

// Macros converted to editable markdown — NOT wrapped in verbatim <!--[CONFLUENCE]--> blocks.
// These have push-side counterparts (GitHub Alerts or {{shortcodes}}) that round-trip correctly.
const NO_WRAP_MACROS = new Set([
  'info', 'note', 'tip', 'warning',                       // → GitHub Alerts on push
  'toc', 'children', 'status',                            // → {{shortcodes}} on push
]);

// Display-only macros: no meaningful markdown equivalent and params must be preserved.
// Wrapped verbatim (no markdown label) so full XML is restored on push via API fixup.
const VERBATIM_ONLY_MACROS = new Set([
  'recently-updated', 'space-details', 'page-tree',
]);
...
function convertMacros(html) {
  // Normalize self-closing macros to have body so the main regex can match them.
  // e.g. <ac:structured-macro ac:name="toc" .../> → <ac:structured-macro ac:name="toc" ...></ac:structured-macro>
  html = html.replace(/<(ac:structured-macro[^>]*?)\/>/g, '<$1></ac:structured-macro>');
  ...
  do {
    ...
    html = html.replace(re, (fullMatch, name, body) => {
      ...
      const converted = convertMacro(name, body);
      if (converted === null) {
        // Display-only macro: wrap verbatim with no markdown label (params must survive round-trip)
        if (VERBATIM_ONLY_MACROS.has(name)) {
          return wrapConfluence(fullMatch);
        }
        // Unknown macro — preserve verbatim so no data is lost on round-trip
        return wrapConfluence(fullMatch, `\`confluence-macro: ${name}\``);
      }
      if (NO_WRAP_MACROS.has(name)) {
        // Editable macro — no verbatim wrap; push side handles round-trip
        return converted.trim();
      }
      // Standard verbatim wrap (code, mermaid, expand, images, etc.)
      return wrapConfluence(fullMatch, converted.trim());
    });
  } while (html !== prev);
  return html;
}
...
function convertLists(html) {
  // Process lists inside-out. The tempered greedy token ((?:(?!<\/?ul|<\/?ol)[\s\S])*)
  // ensures each pass only matches the innermost list (no nested ul/ol in content),
  // so the do-while correctly unwinds from deepest to shallowest.
  let prev;
  do {
    prev = html;
    // Innermost unordered list (content contains no nested ul/ol)
    html = html.replace(/<ul[^>]*>((?:(?!<\/?ul|<\/?ol)[\s\S])*)<\/ul>/gi, (_, content) => {
      const items = [];
      for (const li of content.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)) {
        const item = buildListItem(li[1]);
        if (item !== null) items.push(`- ${item}`);
      }
      return '\n' + items.join('\n') + '\n';
    });
    // Innermost ordered list
    html = html.replace(/<ol[^>]*>((?:(?!<\/?ul|<\/?ol)[\s\S])*)<\/ol>/gi, (_, content) => {
      const items = [];
      let n = 1;
      for (const li of content.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)) {
        const item = buildListItem(li[1]);
        if (item !== null) items.push(`${n++}. ${item}`);
      }
      return '\n' + items.join('\n') + '\n';
    });
  } while (html !== prev);
...
/**
 * Build the text of a single list item from its raw HTML content.
 * - Strips <p> wrappers (Confluence wraps li content in <p> in some modes)
 * - Indents already-converted nested list lines by 2 spaces
 */
function buildListItem(rawHtml) {
  // Strip <p> wrappers, keeping their text content inline
  const stripped = rawHtml.replace(/<p[^>]*>([\s\S]*?)<\/p>/gi, (_, c) => c.trim());
  // Split into lines, drop empty-only lines
  const lines = stripped.split('\n').filter(l => l.trim() !== '');
  if (lines.length === 0) return null;
  const firstLine = lines[0].trim();
  // Remaining lines are sub-list items (already converted by the inner do-while pass).
  // Indent them by 2 spaces so markdown parsers see them as nested.
